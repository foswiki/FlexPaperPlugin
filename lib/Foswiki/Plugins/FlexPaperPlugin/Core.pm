...
sub FLEXPAPER {
    ...
    return inlineError("$theFileName not found at $theWeb.$theTopic")
      unless Foswiki::Func::attachmentExists( $theWeb, $theTopic,
        $theFileName );
    ...
    return inlineError(
"$theFileName not found at $theWeb.$theTopic even though the store says so"
    ) unless -f $origFile;
    ...
    writeDebug(
"origFile=$origFile, origModified=$origModified, swfFile=$swfFile, cachedModified=$cachedModified"
    );

    if ( $origModified > $cachedModified ) {
        writeDebug("generating new swf for $theFileName at $swfFile");
        my $pdf2swfCmd = $Foswiki::cfg{FlexPaperPlugin}{Pdf2swfCmd}
          || 'pdf2swf -q -f -T 9 -t -s storeallcharacters %FILENAME|F% -o %OUTPUT|F%';
    ...
    my $swfUrl =
      ...
      . time();
    my $result =
        "<noautolink><div id='$id' class='jqFlexPaper {"
      . "source:\"$swfUrl\", "
      . "width:\"$theWidth\", "
      . "height:\"$theHeight\", "
      . "Scale:\"$theScale\", "
